...
//Please, have a look at WirelessRGBW.h for information and configuration of Arduino project
...
void setup(void) {
  ...
  Serial.print(F("J6B Wireless RGBW ")); Serial.println(VERSION);
  ...
  Serial.print(F("Start RGBW"));

  webRGBW.Init(config.model);

  Serial.print(F(" : OK\r\nStart WebServer"));
  InitSystemWebServer(server);
  config.InitWebServer(server, shouldReboot);
  webRGBW.InitWebServer(server);
  server.begin();
  Serial.println(F(" : OK"));
